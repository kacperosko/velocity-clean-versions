...
def delete(args):
    ...
    for Sobject, file in metadata.items():
        print(
            clr.OKBLUE + ">> Running ->" + clr.ENDC + f"sf data delete bulk --target-org {args['user']} -s {Sobject} -f ./bin/temp/" + file)
        try:
            if os.system(f"sf data delete bulk --target-org {args['user']} -s {Sobject} -f ./bin/temp/" + file) == 0:
